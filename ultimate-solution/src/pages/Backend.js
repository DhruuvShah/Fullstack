...
function Backend() {
  ...
  const processString = async (operation) => {
    const res = await fetch("/api/string", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ text: stringText, operation }) });
    const data = await res.json();
    setStringResult(data.result);
  };
  const runHello = async () => {
    const res = await fetch("/api/hello");
    const data = await res.json();
    setHelloOutput(data.output);
  };
  const runTimers = async () => {
    const res = await fetch("/api/timers/basic");
    const data = await res.json();
    setTimersOutput(data.output);
  };
  ...
  const readTextInterval = async () => {
    ...
    const data = await res.json();
    setTextIntervalContent(data.content);
  };
  ...
  return <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
      ...
      <GlassCard title="Custom Math Module" icon={Calculator} delay={0.2}>
        <div className="flex flex-col gap-4">
          ...
          {mathResult !== null && <div className="text-center text-2xl font-bold mt-2">Result: {mathResult}</div>}
        </div>
      </GlassCard>
      ...
      <GlassCard title="OS Module" icon={HardDrive} delay={0.3}>
        <div className="flex flex-col gap-4">
          ...
          {osInfo && <div className="bg-black/5 dark:bg-white/5 p-4 rounded-2xl text-sm font-mono overflow-x-auto">
      ...
      <GlassCard title="File System (fs)" icon={FileText} delay={0.4}>
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            ...
            {fileContent && <div className="bg-black/5 dark:bg-white/5 p-3 rounded-xl text-sm font-mono">
                {fileContent}
              </div>}
          </div>
        </div>
      ...
      <GlassCard title="Custom String Module" icon={Type} delay={0.9}>
        <div className="flex flex-col gap-4">
          ...
          {stringResult && <div className="bg-black/5 dark:bg-white/5 p-3 rounded-xl text-center font-medium text-lg">
              {stringResult}
            </div>}
        </div>
      </GlassCard>
