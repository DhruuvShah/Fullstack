...
export function ApiWork() {
  ...
  const handleClickPost = () => { setResource("posts"); alert('Post data Successfully fetched'); };
  const handleClickComment = () => { setResource("comments"); alert('Comment data Successfully fetched'); };
  const handleClickUser = () => { setResource("users"); alert('User data Successfully fetched'); };
  ...
  useEffect(() => {
    ...
    fetch(`https://jsonplaceholder.typicode.com/${resource}`)
      ...
      .then(json => {
        setData(json.slice(0, 2)); // Limit for UI
        setLoading(false);
      });
  }, [resource]);
        ...
        {loading ? (
          <div className="apiwork-skeleton-list">
            <div className="apiwork-skeleton-card">
              <div className="apiwork-skeleton-title"></div>
              <div className="apiwork-skeleton-body"></div>
              <div className="apiwork-skeleton-body-short"></div>
            </div>
            <div className="apiwork-skeleton-card">
              <div className="apiwork-skeleton-title"></div>
              <div className="apiwork-skeleton-body"></div>
