...
const mathModule = require('./backend/unit3/Index.js');
const stringModule = require('./backend/unit4/string.js');
...
async function startServer() {
  ...
  app.post("/api/string", (req, res) => {
    ...
    let result = '';
    switch(operation) {
      case 'uppercase': result = stringModule.uppercase(text); break;
      case 'lowercase': result = stringModule.lowercase(text); break;
      case 'camelcase': result = stringModule.camelcase(text); break;
      case 'capitalize': result = stringModule.capitalize(text); break;
      default: return res.status(400).json({ error: 'Invalid operation' });
    }
    res.json({ result });
  });

  // --- NEW ENDPOINTS FOR MISSING TASKS ---

  // Unit 3, Task 1: Hello Node
  ...
  // Unit 3, Task 6: setTimeout and setInterval
  ...
  // Unit 3, Task 7: setInterval append text
  ...
  // Unit 3, Task 8: setInterval date time
