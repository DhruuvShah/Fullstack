...
function Calculator() {
  ...
  const addNum = (value) => setInput(input + value);
      ...
      <div className="calc-grid">
        {buttons.map((btn) => (
          <motion.button
            ...
            className={`calc-btn ${btn.wide ? "calc-btn-wide" : ""} ${btn.type === "operator" ? "calc-btn-operator" : btn.type === "action" ? "calc-btn-action" : "calc-btn-number"}`}
          >
            {btn.label}
          </motion.button>
        ))}
